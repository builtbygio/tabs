...
(TabBarView = class TabBarView {
  constructor(pane, location) {
    ...
    for (var item of Array.from(this.pane.getItems())) { this.addTabForItem(item); }
  ...
  terminatePendingStates() {
    for (var tab of Array.from(this.getTabs())) { if (typeof tab.terminatePendingState === 'function') {
  ...
  closeSavedTabs() {
    return (() => {
      const result = [];
      for (var tab of Array.from(this.getTabs())) {
  ...
  onDragStart(event) {
    ...
    if (typeof item.getAllowedLocations === 'function') {
      for (var location of Array.from(item.getAllowedLocations())) {
  ...
  onMouseEnter() {
    for (var tab of Array.from(this.getTabs())) {
      var {width} = tab.element.getBoundingClientRect();
      tab.element.style.maxWidth = width.toFixed(2) + 'px';
    }
  }

  onMouseLeave() {
    for (var tab of Array.from(this.getTabs())) { tab.element.style.maxWidth = ''; }
  }
